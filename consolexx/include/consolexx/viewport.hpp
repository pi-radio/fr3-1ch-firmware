...
namespace consolexx
{
  class viewport : public consolexx::termobj {
  ...
  protected:
    ...
    viewport(viewport *parent, position _p, size _s) : termobj(parent),
        _focus(NULL), p(_p), s(_s) { }

    viewport(viewport *parent, rect _r) : viewport(parent, _r.p, _r.s) {}

  public:
    inline rect local2global(const rect &_r) const { if (_parent) return parent<viewport>()->local2global(_r + p); return _r; }
    inline rect global2local(const rect &_r) const { if (_parent) return parent<viewport>()->global2local(_r) - p; return _r; }
